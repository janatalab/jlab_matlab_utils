...
% 2006/12/14 - Petr Janata modified algorithm for determining if dimension is evenly spaced
...
evenx = all(abs(diff(diff(x)))<1e-12);    % PJ
eveny = all(abs(diff(y)/dy-1)<1e-12);     % true if Y is linearly spaced
eveny = all(abs(diff(diff(y)))<1e-12);    % PJ
