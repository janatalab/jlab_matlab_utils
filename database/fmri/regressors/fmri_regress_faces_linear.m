function [names,vals] = fmri_regress_faces_linear(pinfo,minfo,sess)

% generates linear blocked fmri data regressors for the faces mood induction task
% 
%   [names,vals] = fmri_regress_faces_linear(pinfo,minfo,sess)
% 
...
rnum = pinfo.irun;
if isempty(strfind(pinfo.presfname,'faces')), return, end
vidx = strfind(pinfo.presfname,'faces')+6;
...
rfilt.include.all.EVENT_CODE = {'faces_.*_block.*'};
...
for j=1:nons
  names{j} = sprintf('faces_%s_lin%d',emo,j);
