...
%   defs.init.USE_FSL
%   defs.init.CLOBBER
%   defs.init.run_dir_prefix - specifies what the prefix string for run
%             directory names should be. {'subject','session','none'}.
%             default is subject, in which case <subid>_ is prefixed before
%             run1, run2, etc. 'none' indicates that only run1, run2 should
...
try run_dir_prefix = defs.init.run_dir_prefix; catch run_dir_prefix = {'subject'}; end
		...
		for irun = 1:nruns
			...
			if SYMLINK_EPIS
				...
				outdata.data{paths_idx} = ensemble_add_data_struct_row(...
					...
					'run',runidx,'path_type','run_indir','path',run_indir);
				
				have_run_indir = ~isempty(dir(fullfile(run_indir,'*.img')));
			else
				have_run_indir = 0;
			end % if SYMLINK_EPIS
			
			% Figure out what the run directory prefix should be
			run_prefix = '';
			for ipref = 1:length(run_dir_prefix)
				switch run_dir_prefix{ipref}
					case 'subject'
						run_prefix = sprintf('%s%s_', run_prefix, subid);
					case 'session'
						run_prefix = sprintf('%s%s_', run_prefix, session_stub);
				end
			end
			run_outdir = fullfile(epi_outdir, sprintf('%srun%d',run_prefix,runidx)); % location of modified data
			...
			if ~isempty(dir(fullfile(run_outdir,'*.img'))) || have_run_indir ...
					 || using_resid
				...
				if VOL_CHECK
					...
					% See if we already have an output file and if we want to clobber
					% it
					skip_vol_check = 0;
					if exist('WRITE2FILE','var') && WRITE2FILE
						check_dir(defs.paths.figpath,1);
						fvc_fname = fullfile(defs.paths.figpath,sprintf('fmri_vol_check_%s_%s.ps',...
							subid, session_stub));
						pdf_fname = fullfile(defs.paths.figpath,sprintf('fmri_vol_check_%s_%s.pdf',...
							subid, session_stub));
						if exist(pdf_fname) && ~CLOBBER
							fprintf('Found volcheck file: %s, skipping ...\n', pdf_fname);
							skip_vol_check = 1;
						end
					end
					...
					if ~skip_vol_check
						...
						% If files are named using session names rather than top-level
						% subject names, check this.
						if ~npts
							flist = dir(fullfile(run_outdir,sprintf('%s*.img',session_stub)));
							npts = length(flist);
						end
						...
						if isfield(defs.init,'figs') && isfield(defs.init.figs,'display') ...
								&& defs.init.figs.display
							if irun == 1
								figure(1), clf
							end
							...
							if irun == 1
								title(sprintf('Subject %s: Session %s: Run %d',...
									sinfo(isub).id,session_stub,runidx));
							else
								title(sprintf('Run %d', runidx));
							end
							
							if irun == nruns && exist('WRITE2FILE','var') && WRITE2FILE
								msg = sprintf('Writing mean intensity plot to %s',fvc_fname);
								r = update_report(r,msg);
								print(fvc_fname,'-dpsc');
				...
				if REPLACE_BAD_VOLS && isfield(infost,'badvols') && ...
					...
					nbad = size(infost.badvols{runidx},1);
					for ibad = 1:nbad
						badvol_idx = infost.badvols{runidx}{ibad,1};
						goodvol_idxs = infost.badvols{runidx}{ibad,2};
				...
				if exist('flist') && ~isempty(flist) && exist('epi_idx','var')
					for ifl=1:size(flist,1)
						outdata.data{epi_idx} = ensemble_add_data_struct_row(...
							...
							isess,'ensemble_id',...
							sess.ensemble_id,'run',runidx,'path',flist(ifl,:));
					end
				end
