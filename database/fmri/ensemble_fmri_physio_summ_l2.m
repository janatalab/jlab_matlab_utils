function outdata = ensemble_fmri_physio_summ_l2(indata,defs)

% calculates group-level summary statistics for physio data collected during fMRI
% 
% REQUIRES
%   indata - ensemble data struct containing the following structs:
%       {gsr|cardiac}_epochs - output from ens_fmri_physio_summ, containing
%           responses and epoched, scored signals to use to construct
%           group-level mean waveforms
%       paths
%       sinfo
% 
%   params.{gsr|cardiac}_epochs.filter - filtering criteria to apply to
%       either/or gsr_epochs/cardiac_epochs
%   params.{gsr|cardiac}_epochs.transform_function - string, function
%       handle, or cell array of strings and function handles to apply to
%       the {gsr|caridac}_epochs data struct before plotting. possible
%       function could include one that applies a log(SCL+1) or
%       (SCL-SCLmin)/(SCLmax-SCLmin) transformation within or between
%       subjects
% 
% FB 2009.04.21
...
try pkh.transformation = ps.peak_heights.transformation;
catch pkh.transformation = 'none'; end
