	...
	for isess = 1:nsess
		...
		if BUILD_MODEL
			...
			if exist('pres_paths','var')
				...
				presfilt.include.all.session = {sessid};
			...
			if exist('epidata','var')
				...
				epiFilt.include.all.session = {sess.id};
			...
			if nruns == 0
				error('Have no run information in the session data');
			end
					...
					if USE_FSL
						...
						if combine_runs
							...
							outdata.data{mod_idx} = ensemble_add_data_struct_row(outdata.data{mod_idx},...
								'subject_id',subid,'session',sessid,'model_id',model_id,'run',runnum,...
										...
										if ~isempty(curr_cond_idx)
											...
											elseif nnewmod
												...
												msg = sprintf(['new parametric modulations for previously '...
													...
													'run %d'],allconds{icond},sessid,runnum);
												r = update_report(r,msg);
			...
			if combine_runs
				if USE_SPM
					...
					if ESTIMATE_MODEL
						...
						outdata.data{res_idx} = ensemble_add_data_struct_row(outdata.data{res_idx},...
							'subject_id',subid,'session', sessid,'model_id',model_id,'run',irun,...
							'path',resid_fname);
					end
				elseif USE_FSL
					...
					evstub = sprintf('%s_%s_ev%%d.txt',subid, sessid);
					for iev=1:nev
					...
					epifname = fullfile(epi_outdir,sprintf('%s_%s_concat_all',...
						subid, sessid));
					...
					outdata.data{mod_idx} = ensemble_add_data_struct_row(outdata.data{mod_idx},...
						'subject_id',subid,'session',sessid,'model_id',model_id,...
					...
					if PLOT_DESMAT_COV
						...
						opts.printfig = 1;
						opts.title = sprintf('%s, session %s, model %02d', subid, sessid, model_id);
...
if PLOT_DESMAT_COV && USE_SPM
	for j=1:length(outdata.data{mod_idx}.data{mcols.path})
		...
		sessid = outdata.data{mod_idx}.data{mcols.session}(j);
...
if PERMUTE_MODEL && USE_SPM && BUILD_MODEL && ESTIMATE_MODEL
	...
	for j=1:length(outdata.data{mod_idx}.data{mcols.path})
		...
		while (~done)
			...
			% plot permutations
			pp.pfigtitle = sprintf('%s, session %s, model %d, iteration %d',...
				subid,sessid,model_id,iter);
		...
		figfname = fullfile(defs.paths.figpath,...
			sprintf('%s_%s_mod%d_permuted.ps',subid,sessid,model_id));
