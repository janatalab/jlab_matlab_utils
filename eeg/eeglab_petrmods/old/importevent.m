...
for curfield = tmpfields'
    ...
    switch lower(curfield{1})
        ...
        case 'event', % load an ascii file
			...
			% August 8, 2005 - Petr Janata
			% The code was only copying the first letter of strings
			% that were being passed as event type names
...
% 			   end;
% 			else 
% 			  % event = setstruct( event, g.fields{eventfield}, g.indices, cell2mat(tmparray(:,eventfield)));
% 			end;
                      end;      
					  % generate ori fields
					  % -------------------
...
% ---------------------------While this architecture allows for an extensible framework of functions to be built which support the computations and properties associated with that node, the current version solely implements assignment of waveforms.
