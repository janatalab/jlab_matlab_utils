...
if 0,
		...
		% filesize, position of eventtable, headerlength, etc. 	
			...
			% Identify epoch onsets
			ix1 = diff([0;bitand(t,2^16)]);	
						
			label_resolution = 2^15-1;  % events can be encoded by
                                                    % the bottom 15 bits
			event_trailing_edge = 2^15; % Bit 16 being "on"
                                                    % designates event
                                                    % offset.			
			ix2 = diff([0;bitand(t,label_resolution)]);	% labels 
			
			%HDR.BDF.ANNONS = t; 	% debugging information 
                        %EVENTTABLE = repmat(0,sum(~~ix1)+sum(~~ix2),4);
                        
			% Ignore epoch markers. These belong in the status byte
			% anyway, not in the trigger lines
			
                        %POS = [find(ix1>0);find(ix2>0); ...
			%      find(ix1<0);find(ix2<0)];
			POS = [find(ix2>0); find(ix2<0)];
			
			% Create an array of event types with onsets and then
			% offsets.  Fixed the code for getting the correct
			% event code for the trailing edge
                        TYP = [bitand(t(ix2>0),label_resolution); ...
			      bitand(t(find(ix2<0)-1),label_resolution)+event_trailing_edge];
