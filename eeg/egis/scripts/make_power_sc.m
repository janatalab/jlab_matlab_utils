	...
	for ifile = 1:size(csdmfiles,1)
	  ...
	  for iwin = 1:nwin
		...
		elseif strcmp(output,'norm')
		  row_av = sum(power_data')'./((ave_fhdr(NChan)-size(find(bad_chan(ifile,:)),2))*ones(size(power_data,1),1));
		  power_data(1:npts,:) = 500*power_data(1:npts,:) ./ (row_av*ones(1,size(power_data,2)));
		  scale_values(is,:) = row_av';
		  is = is + 1;
