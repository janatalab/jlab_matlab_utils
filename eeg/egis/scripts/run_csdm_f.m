...
if ((nargin == 6)&(length(avg_cells) == 0))
	avg_cells = [1:ses_fhdr(NCells)];
end
...
for c = 1:num_cells_avg
	...
	cell_offset_mask(c) = sum(ses_chdr(1:avg_cells(c), NTrials)) - ses_chdr(avg_cells(c), NTrials);
	for t=1:ses_chdr(avg_cells(c),NTrials)
...
for c=1:num_cells_avg
	accessed_cell = 0;
	thecell = avg_cells(c);
	disp(['Processing cell ' int2str(thecell)]);
	for iwin = 1:nwin
%		disp(['  Observation ' int2str(iwin)]);
		...
		for t=1:ses_chdr(thecell, NTrials)
			...
			%	call to csdm
				[power_trialdata,fft_trialdata] = csdm(avref_trialdata(start_samp:stop_samp,:));		
				fwrite(sfid,real(fft_trialdata(single_trial_f,:)),'float');
				fwrite(sfid,imag(fft_trialdata(single_trial_f,:)),'float');
		...
		num_good_trials = sum(ses_mask(start_offset:stop_offset,:));
