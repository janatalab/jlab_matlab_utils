...
if ~((size(samps,1) == size(cells,1))&(size(samps,2) == size(cells,2)))
...
% WORK THRHOUGH EACH observation
for iobs = 1:size(obs,2)
  disp(['Working on observation: ' int2str(obs(iobs))]);
  image_binary = zeros(plot_res*size(cells,1),plot_res*size(cells,2));
  for icell=1:size(cells,1)
	    for t=1:size(cells,2)
	    c = cells(icell,t);
	    disp(['  Cell: ' int2str(c)]);
	    disp(['    Samp: ' int2str(samps(icell,t))]);
   	    trialdata = rd_onetr_allch(srcfid, coff(c), obs(iobs), fhdr(NChan), chdr(c, NPoints));
        w = trialdata(samps(icell,t),:);
        good_chan = find(good_chan_mask(obs(iobs),:));
  ...
  outfname = [output_root '_o' int2str(obs(iobs))];
