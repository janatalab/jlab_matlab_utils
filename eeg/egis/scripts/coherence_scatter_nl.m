...
if size(obs,2) > 1
	for g = 1:size(arrayg,1)
			for i= 1:size(freqs,2)
				...
				hold on
				for io = 1:size(obs,2)
					if ~strcmp(deblank(setstr(reference(obs(io),:))),'laplacian')
						hold on,plot(dist,bcohpotential,'k-');
					else 
						hold on,plot(dist,bcohlaplacian,'k-');
					end;
				end
				iline = 1;
				...
				hold on, plot([0:1:max(twod_dist(pairs))],coh_fit,line_type(iline,:)), axis([0 max(twod_dist(pairs))+1 0 1])
				title(['Coherence versus Distance:  @' int2str(freqs(i)) ' Hz ' hnames(g,:)]);
				text([max(twod_dist(pairs))-1],[1-0.1*j],[scat_type(iline,2) ' ' deblank(setstr(obs_labels(obs(j),:)))])
	...
	for g = 1:size(arrayh,1)
		...
		for io = 1:size(obs,2)
			if ~strcmp(deblank(setstr(reference(obs(io),:))),'laplacian')
				hold on,plot(dist,bcohpotential,'w-');
			else
				hold on,plot(dist,bcohlaplacian,'w-');
			end;
		end
		for j = 1:size(freqs,2)
			...
			text([max(twod_dist(pairs))-1],[1-0.1*j],[scat_type(iline,2) ' ' int2str(freqs(j)) ' Hz'])
			xlabel('Interelectrode Distance (cm)')
			ylabel('Coherence')
			iline = iline+ 1;
		end
